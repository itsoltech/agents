...
9. Plan approval must be explicit after the user saw the specific plan. Do not infer approval from the original task request, "continue", "direct user request", silence, or a generic main-agent statement.
10. For bugfixes, require evidence, Fix Decision Gate before plan writing, and an approved Technical Fix Plan before implementation.
11. Require Business Plan approval, Technical Plan approval, and execution-mode choice before implementation.
12. Load the focused ITSOL workflow and domain skills for those surfaces.
13. Ask only for missing information that cannot be inferred safely; if the missing information changes scope, behavior, architecture, rollout, data, permissions, or UX, ask before planning.
