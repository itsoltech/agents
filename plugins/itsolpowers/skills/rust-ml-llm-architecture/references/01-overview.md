# rust-ml-llm-architecture Reference Sector: Overview
...
Ten dokument opisuje dobre praktyki dla aplikacji ML / LLM pisanych w Rust z użyciem Rig i Candle. Rig traktujemy jako warstwę orkiestracji inferencji LLM, agentów, narzędzi, RAG, embeddingów i integracji z providerami. Candle traktujemy jako runtime i framework ML do lokalnej inferencji, treningu, fine-tuningu, ładowania wag, pracy z tensorami, GPU i modelami z Hugging Face.

Dokument jest niezależny od frameworka backendowego. Te same zasady można stosować, gdy API jest pisane w Axum, Actix, Poem, Tonic, Effect, Node albo innym runtime. Granice systemu powinny być opisane kontraktami, typami, telemetryką i politykami bezpieczeństwa, a nie konkretną biblioteką HTTP.
