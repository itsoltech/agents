# hey-api-openapi-codegen Reference Sector: Overview
...
Ten dokument opisuje zasady korzystania z `@hey-api/openapi-ts` w projektach frontendowych i TypeScriptowych. Obejmuje konfigurację generatora, jakość specyfikacji OpenAPI, generowanie typów, klienta HTTP, SDK, walidację runtime, integrację z TanStack Query, Svelte/SvelteKit, bezpieczeństwo, CI i review zmian.

Generator nie zastępuje poprawnego kontraktu API. Jeśli specyfikacja OpenAPI jest nieprecyzyjna, wygenerowany klient będzie tylko typowaną reprezentacją tej nieprecyzyjności. Największa wartość z codegen pojawia się wtedy, gdy backend, frontend i CI traktują OpenAPI jako kontrakt, a nie jako dokumentację pisaną po fakcie.
## Założenia
