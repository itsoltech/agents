...
10. Stop and get explicit user approval for the Business Plan file.
11. Before writing the Technical Plan, run a Technical Decision Gate: inspect repo constraints, verify current technology context when relevant, present feasible technical approaches with tradeoffs, recommend one, and ask the user to choose or approve the approach.
12. Write the Technical Plan to a repo-local markdown file that references the approved Business Plan and confirmed technical approach.
13. Self-review the Technical Plan file for missing files, missing skills, missing Current Tech Context, weak TDD steps, vague logic, unapproved technical choices, TODOs, verification gaps, and unresolved risks; fix issues before asking for approval.
14. Stop and get explicit user approval for the Technical Plan file.
15. Ask whether execution should run with subagents or inline.
16. If the user chooses subagent-driven execution, proceed through `itsol-subagent-workflow` with both plan file paths.
17. If the user chooses inline execution, proceed to `itsol-feature-implementation` and `itsol-tdd-workflow` with both plan file paths.
