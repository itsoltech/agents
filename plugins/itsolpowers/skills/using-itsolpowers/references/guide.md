...
- Review PR: użyj `itsol-code-review-workflow`, zbuduj mapę obszarów review i dobierz skille review dla dotkniętych technologii. Dla dużego, wieloobszarowego albo ryzykownego PR użyj osobnych sub-agentów według obszarów, np. security, infra, frontend, backend, database, generated clients/API contracts, migration/rewrite, QA/release, performance i test strategy. Inline-only review dopuszczaj tylko dla małego jednoobszarowego diffu i jawnie uzasadnij, dlaczego sub-agenci nie byli potrzebni.
