...
4. Load `itsol-execution-policy` after workflow mode. Resolve the separate preset, policy sources, model/reasoning intent and control, distinct-agent/parallel/review ceilings, observable `done_when`, ranked `stop_after`, and escalation behavior. Default to `standard`; do not invent a numeric total-agent ceiling without explicit user or repository authority. Never use `maxTurns` or treat an agent stopping as task completion.
