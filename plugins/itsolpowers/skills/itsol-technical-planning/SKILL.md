...
4. Capture the selected approach, rejected alternatives, risks, open questions, owners, and verification plan.
5. Prefer the simplest safe vertical slice; avoid hidden scope, unbounded refactor, and unverifiable estimates.
6. Include concrete files/modules, current tech context, required ITSOL skills, logical branches, TDD entry points, verification commands, candidate subagent split, and a proposed subagent concurrency limit when subagent-driven execution is likely.
7. For risky release work, document deployment order, post-release validation, monitoring, rollback, and responsible people.
8. Require explicit user approval of the Technical Plan and then ask whether execution should be subagent-driven or inline.
