# tanstack-query-svelte-implementation Reference Sector: `useMutationState`
...
`useMutationState` pozwala obserwować mutacje poza komponentem, który je uruchomił.

Przykładowe zastosowania:

- globalny licznik zapisujących się zmian
- lista optimistic items dodawanych w innym miejscu UI
- blokowanie nawigacji, gdy trwa zapis
- zbiorcze pokazywanie błędów mutacji

Zasady:

- dodawaj `mutationKey`, jeśli mutacja ma być obserwowana globalnie
- filtruj mutacje po `mutationKey` i statusie
- nie buduj z `useMutationState` głównego modelu danych aplikacji
- traktuj to jako obserwację stanu mutacji, nie jako trwały cache danych
## Paginacja
