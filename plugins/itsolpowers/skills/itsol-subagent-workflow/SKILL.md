...
Build a dependency-aware task graph, set a concurrency limit, give bounded read/write scope, enforce one writer per file/shared contract, require TDD or documented replacement verification, validate every response, and use independent review for changed surfaces. Initiative phase packets also carry `initiative_id`, `phase_id`, relevant requirement IDs, canonical artifact paths, current decisions, and phase-level evidence obligations; child completion updates neither initiative nor phase state until the main agent validates and records it. Preserve `partial`, `blocked`, `failed`, unverified, and coverage-gap results. Keep integration and final validation with the main agent.
