# postgres-operations-debugging Reference Sector: PgBouncer - tryby pracy
...
- server connection jest przypisany do klienta na cały czas trwania połączenia klienta
- semantyka jest najbardziej zbliżona do direct connection
- działają session-level features: prepared statements, temporary tables, `SET`, `LISTEN/NOTIFY`, session advisory locks, cursory zależne od sesji
- redukcja połączeń do PostgreSQL jest mniejsza, bo długi idle client może trzymać server connection
- ten tryb jest dobry dla aplikacji wymagających pełnej kompatybilności z PostgreSQL
- ten tryb jest dobry dla narzędzi administracyjnych, migracji, BI i części ORM-ów z trudnym do zmiany session state

### Transaction pooling

- server connection jest przypisywany tylko na czas transakcji
- po zakończeniu transakcji server connection wraca do puli i może obsłużyć innego klienta
- ten tryb daje największą korzyść przy krótkich transakcjach i wielu klientach
- klient nie może zakładać, że dwa kolejne zapytania trafią na tę samą sesję PostgreSQL
- `SET`, temp tables, prepared statements bez wsparcia PgBouncera, session advisory locks i `LISTEN/NOTIFY` nie powinny być używane jako trwały stan między transakcjami
- każda operacja, która wymaga stałego kontekstu, musi być zamknięta w jawnej transakcji albo przeniesiona na direct/session connection
- `server_reset_query` nie jest domyślnie używany w transaction pooling, bo aplikacja nie powinna zależeć od session state w tym trybie
- jeśli aplikacja wymaga `SET LOCAL`, cała operacja musi działać wewnątrz jednej jawnej transakcji

### Statement pooling

- server connection wraca do puli po każdym zapytaniu
- multi-statement transactions są niedozwolone
- tryb jest najbardziej agresywny i najmniej kompatybilny
- używaj tylko dla bardzo prostych workloadów bez transakcji wielozapytaniowych
- nie używaj jako domyślnego trybu dla aplikacji biznesowych
## PgBouncer vs direct connection
