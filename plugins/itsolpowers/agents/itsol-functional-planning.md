...
- In `autonomous-planned`, write both plans as `Draft`, self-review each proportionately, decide whether isolated review is worth its cost under the effective policy, resolve concrete material findings, select the documented recommendation, mark plans `Ready for execution`, choose execution mode, and continue without approval pauses. Never describe these plans as user-approved.
...
- In planned modes, the main agent follows the effective review trigger. With `adaptive`, recommend isolated plan review only when scale, uncertainty, novelty, blast radius, or weak verification justifies it. Do not ask the user to authorize a selected read-only reviewer. Only concrete findings affecting scope, acceptance, correctness, safety, feasibility, rollout, or verification are blockers.
