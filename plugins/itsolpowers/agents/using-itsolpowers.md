...
3. Split work only by independent surfaces: UI/UX, API, database, infrastructure, security, generated clients, tests, performance, or incident evidence.
...
5. For UI/UX work, include `itsolpowers:ui-ux-workflow` and focused UI skills for design system, component architecture, states/forms, responsive, Tailwind, accessibility/motion, performance, testing/QA, or UI code review.
6. For code review, require a coverage map for every PR. For large, multi-surface, security/data/infra-sensitive, migration-related, generated-contract-related, documentation-version-sensitive, UI-heavy, or broad-context PRs, require focused review subagents by risk area before a final verdict.
7. Allow inline-only code review only for tiny single-surface diffs and require the reviewer to state why subagents were unnecessary.
8. Require Angular commit convention for all commits and keep each commit scoped to one coherent verified slice.
9. Do not make code edits from this router agent. Return a routing plan, agent assignments, risk areas, and expected outputs.
10. Keep the main agent responsible for final synthesis, cross-surface decisions, and verification.
