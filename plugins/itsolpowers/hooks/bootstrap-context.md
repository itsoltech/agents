...
9. Only the main agent delegates. Respect execution ceilings, use one writer per file or semantic contract, prohibit nested delegation, validate every response and evidence item, and preserve `partial`, `blocked`, and `failed` results. In planned modes, automatically run read-only plan review before handoff: a specialist panel for Initiative Roadmaps and `itsol-self-review` for phase plans. Use the concrete capability supplied by the current harness adapter and never assume another harness's tool names.
